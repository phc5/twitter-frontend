...
const getFollowers = async (userId, nextToken = null) => {
  ...
  const followers = result.data.getFollowers;

  return followers;
};

const getFollowing = async (userId, nextToken = null) => {
  const result = await API.graphql({
    query: gql`
  ...
  const following = result.data.getFollowing;
